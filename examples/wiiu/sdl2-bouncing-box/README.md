...
a Wii U console named `wiiu` on the local network.

The rule that converts `.elf` into `.rpx` is generated by the `configure` script, inside
`aminclude.am`. We import those rules using the `@INC_AMINCLUDE@` line.
